...
export default function StaffExchanges() {
  ...
  const fetchExchanges = async () => {
    try {
      setLoading(true);
      const res = await staffApi.getExchanges({ page: 0, size: 50 });
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      await staffApi.updateExchangeStatus(id, status);
      fetchExchanges();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900">Exchange Processing</h1>
